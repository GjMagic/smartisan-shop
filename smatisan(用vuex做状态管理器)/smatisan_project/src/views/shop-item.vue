<template>
  <div class="item">
    <div>
      <div class="item-img">
        <img :alt="listItemData.title" :src="listItemData.ali_image" style="opacity: 1;">
      </div>
      <h6>{{listItemData.title}}</h6>
      <h3>{{listItemData.sub_title}}</h3>
      <div class="params-colors">
        <ul class="colors-list">
          <li v-for="(item,i) in itemSkuInfo" :key="i">
            <a href="javascript:;" :class="{active: i === index}" @mouseenter="smallIcon(i)">
      ...
      <div class="item-btns clearfix">
        <span class="item-gray-btn">
          <router-link :to="{name: 'ItemInfo', query: {itemId: listItemData.sku_id}}">查看详情</router-link>
        </span>
        <span 
          class="item-blue-btn" 
      ...
      <div class="item-cover">
        <router-link :to="{name: 'ItemInfo', query: {itemId: listItemData.sku_id}}"></router-link> 
      </div>
    </div>
  </div>
</template>
